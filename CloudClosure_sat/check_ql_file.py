...
def main():
    ...
    # files_ = files
    # files_ = [0]
    files_ = [0, 1*hour, 2*hour, 3*hour, 4*hour, 5*hour, 6*hour]        # DYCOMS RF02
...
def plot_mean(var_name, files_, zrange, path):
    ...
    for t in files_:
        ...
        plt.plot(var_mean_fields[:], zrange, color=cm1(count_color/len(files_)), label='t=' + str(it) + 's (from field)')
        plt.plot(var_mean[tt, :], zrange, '--', color=cm2(count_color/len(files_)), label='t=' + str(tt * dt_stats) + 's (from Stats)')
        count_color += 1.0
    mini = np.min([np.amin(var_mean[tt,:]),np.amin(var_mean_fields)])
    maxi = np.max([np.amax(var_mean[tt,:]),np.amax(var_mean_fields)])
    # Bomex 170314
