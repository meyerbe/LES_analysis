...
def plot_samples(type, data, data_ql, samples, samples_ql, var_name1, var_name2, scaler, ncomp, z, path):
    ...
    plt.subplot(2, 3, 4)
    plt.scatter(data[:, 0], data[:, 1], c=data_ql[:], s=6, alpha=0.5, edgecolors='none')
    plt.xlim(xmin, xmax)
    plt.ylim(ymin, ymax)
    if type == 'norm':
        plt.title('data norm')
    ...
    try:
        plt.scatter(samples[:, 0], samples[:, 1], c=color_arr[:], s=6, alpha=0.5, edgecolors='none')
    except:
        try:
            plt.scatter(samples[:, 0], samples[:, 1], c=samples_ql[:], s=6, alpha=0.5, edgecolors='none')
        except:
            plt.scatter(samples[:, 0], samples[:, 1], s=6, alpha=0.5, edgecolors='none')
            pass
    plt.xlim(xmin, xmax)
    plt.ylim(ymin, ymax)
    plt.title('samples')
    ...
    if type == 'norm':
        ...
        savename = 'sample_figure_' + 'ncomp' + str(ncomp) + str(z) + 'm.png'
    plt.savefig(
