...
def main():
    ...
    nz = zrange.shape[0]
    error_ql_env = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_ql_rel_env = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_cf_env = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_cf_rel_env = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_ql_domain = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_ql_rel_domain = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    error_cf_domain = np.zeros((len(Lx_range), len(dz_range), nz, ncomp_max))
    ...
    for i in range(len(Lx_range)):
        ...
        for j in range(len(dz_range)):
            print('')
            delta_z = dz_range[j]
            file_name = 'PDF_cond_' + type_ + '_allcomp_Lx' + str(Lx) + 'Ly' + str(Lx) + '_dz' + str(delta_z) + '_time' + str(time_field) + '.nc'
            print(file_name)
            fullpath_in = os.path.join(path_in, file_name)
            print(fullpath_in)
            rootgrp = nc.Dataset(fullpath_in, 'r')
            error_ql_env[i, j, :, :] = rootgrp.groups['error'].variables['error_ql_env'][:, 0:ncomp_max]
            error_ql_rel_env[i, j, :, :] = rootgrp.groups['error'].variables['rel_error_ql_env'][:, 0:ncomp_max]
            error_ql_domain[i, j, :, :] = rootgrp.groups['error'].variables['error_ql_domain'][:, 0:ncomp_max]
            error_ql_rel_domain[i, j, :, :] = rootgrp.groups['error'].variables['rel_error_ql_domain'][:, 0:ncomp_max]
            ...
            error_cf_env[i, j, :, :] = rootgrp.groups['error'].variables['error_cf_env'][:, 0:ncomp_max]
            error_cf_rel_env[i, j, :, :] = rootgrp.groups['error'].variables['rel_error_cf_env'][:, 0:ncomp_max]
            error_cf_domain[i, j, :, :] = rootgrp.groups['error'].variables['error_cf_domain'][:, 0:ncomp_max]
            error_cf_rel_domain[i, j, :, :] = rootgrp.groups['error'].variables['rel_error_cf_domain'][:, 0:ncomp_max]
    ...
    markers = ['o', 'v', '*', 'd', 'p']
    path_out = os.path.join(path, 'PDF_cond_t6h_figures')
    ...
    plot_ql_rel_error_allres_ncompmax(ql_mean_les, ql_mean_stats, ql_mean_domain, ql_mean_env,
                                  error_ql_env, error_ql_rel_env,
                                  zrange, ncomp_max, xlimits_ql, xlimits_cf, dz, Lx_range, dz_range, markers, time_field, path_out)
    plot_cf_error_allres_ncompmax(cf_les, cf_stats, cf_domain, cf_env, error_cf_env, error_cf_rel_env,
                                  zrange, ncomp_max, xlimits_ql, xlimits_cf, dz, Lx_range, dz_range, markers, time_field, path_out)
...
def plot_mean_ql_allres(ql_mean_les, ql_mean_domain, ql_mean_stats, ql_mean_env,
                        zrange, ncomp_max, xlimits_ql, dz, Lx_range, dz_range, markers, time, path):
...
def plot_mean_cf_allres(cf_les, cf_domain, cf_stats, cf_env,
                        zrange, ncomp_max, xlimits_cf, dz, Lx_range, dz_range, markers, time, path):
    cm1 = plt.cm.get_cmap('viridis')
    cm2 = plt.cm.get_cmap('bone')
    cm3 = plt.cm.get_cmap('winter')
    path_out = os.path.join(path, 'error_profiles')
...
def plot_ql_fromsampling_allres_domainmod(factor, ql_mean_les, ql_mean_stats, ql_mean_domain, ql_mean_env, ql_mean_pdf, zrange,
    ...
    for nl in range(n_lx):
        ...
        plt.ylabel('height z (m)')
    plt.suptitle('<ql> from PDF sampling ' + '(t=' + str(time_field) + ')')
    save_name = 'ql_sampling_scaled__dz_nc' + str(ncomp_max) + '_time' + str(time_field) + '.pdf'
    # plt.show()
    plt.savefig(os.path.join(path_out, save_name))
    plt.close()
    ...
    for nl in range(n_lx):
        ...
        plt.ylabel('height z (m)')
    plt.suptitle('<ql> from PDF sampling ' + '(t=' + str(time_field) + ')')
    save_name = 'ql_sampling_scaled_dz_nc' + str(ncomp_max) + '_time' + str(time_field) + '.pdf'
    # plt.show()
    plt.savefig(os.path.join(path_out, save_name))
    plt.close()
...
def plot_ql_error_allres_ncompmax(ql_mean_les, ql_mean_stats, ql_mean_domain, ql_mean_env,
                                  error_ql_env, error_ql_rel_env,
                                  zrange,ncomp_max, xlimits_ql, xlimits_cf, dz, Lx_range, dz_range, markers, time, path):
    print('')
    print('plot error ql')
    # data = (Lx x dz x nz x ncomp)
...
def plot_ql_rel_error_allres_ncompmax(ql_mean_les, ql_mean_stats, ql_mean_domain, ql_mean_env,
    ...
    for nl in range(n_lx):
        ...
        plt.ylabel('height z (m)')
    plt.suptitle('relative error <ql>' + '(t=' + str(time) + ')')
    save_name = 'error_ql_rel_domain_allres_dz_nc' + str(ncomp_max) + '_time' + str(time) + '.pdf'
    # plt.show()
    plt.savefig(os.path.join(path_out, save_name))
    plt.close()
