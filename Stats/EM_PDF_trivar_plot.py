...
def main():
    ...
    d = files[0]
    var = var_list[0] + var_list[1] + var_list[2]
    ...
    for var in var_list:
        ...
        for d in files:
            ...
            '''(2a) plot Data and Histogram'''
            for i in range(len(zrange)):
                iz = zrange[i]
                # plot_PDF_samples(data, var_name1, var_name2, clf, time, z)
                plot_PDF_samples(var+'_mean_sorting', np.int(d[0:-3]), iz * dz)
            ...
            '''(3a) plot Data and Histogram'''
            for i in range(len(zrange)):
                iz = zrange[i]
                # plot_PDF_samples(data, var_name1, var_name2, clf, time, z)
                plot_PDF_samples(var + '_Varw_sorting', np.int(d[0:-3]), iz * dz)
