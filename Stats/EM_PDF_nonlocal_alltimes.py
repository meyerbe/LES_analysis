...
def main():
    ...
    if case_name == 'DCBLSoares':
        ...
        var_list = ['w', 'thetali', 'qt', 'temperature']
        # var_list = ['w']
    ...
    for var in var_list:
        for k in range(zmax):
            ...
            clf = Gaussian_mixture_univar(data, ncomp, var, np.int(d[0:-3]), zrange[0] * dx[2])
            ...
            plot_covar_matrix(covar_tot, corr_tot, zrange_m, zrange_m, var, ncomp, zrange[0] * dx[2])
...
#----------------------------------------------------------------------
def plot_covar_matrix(covar, corr, x_, y_, var_name, ncomp, z):
